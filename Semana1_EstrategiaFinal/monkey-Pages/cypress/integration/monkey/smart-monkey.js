...
function testInitial(){    
		...
		cy.task('genericLog', {"message":`Seed2: ${seed}`})
               
        cy.get('form').within(() => {
            cy.get('.email').type(`${Cypress.env("ghost_email")}`)
            cy.get('.password').type(`${Cypress.env("ghost_password")}`)
            cy.get('.login').click()
        })
		cy.wait(1000)	
}
        ...
        if($inputs.length > 0){
            ...
            if(!Cypress.dom.isHidden(inp)) {
                ...
                if(inp.getAttribute("type") == "email"){
					...
					cy.task('genericLog', {"message":`type Email: ${type}`})
...
function randomEvent(){
    ...
    if(pending_events[typeIndex] > 0){
		...
		let name = functions[typeIndex][fIndex]
		cy.task('genericLog', {"message":`Numero funcion: ${typeIndex}, ${fIndex}`})
		//cy.task('genericLog', {"message":`Nombre funcion: ${name}`})
